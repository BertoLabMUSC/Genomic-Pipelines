...
DBDIR=/U5/Stefano/Genomes/UCSC/hg38_STAR_GencodeV24/
RSEMDIR=/U5/Stefano/Genomes/UCSC/hg38_RSEM_GencodeV24/hg38
...
STAR --runThreadN 14 \
--genomeDir /U5/Stefano/Genomes/UCSC/hg38_STAR_GencodeV24/ \
--readFilesIn $file \
--readFilesCommand zcat \
--sjdbGTFfile /U5/Stefano/Genomes/UCSC/hg38_STAR_GencodeV24/gencode.v24.annotation.gtf \
...
split_bam.py -i $file -r /U5/Stefano/Genomes/UCSC/hg38_STAR_GencodeV24/GRCh38_rRNA.bed -o "$samname"
echo $samname
done
...
geneBody_coverage.py -i $file -r /U5/Stefano/Genomes/UCSC/hg38_STAR_GencodeV24/hg38_UCSC_knownGene.bed -o COVERAGE/"$newname"
echo $file
echo $newname
done
...
# You need the hg38_UCSC_knownGene.bed from RseqQC data base
mkdir DISTRIBUTION/
find . -name "*.UNIQUE.bam" | xargs -n 1 -P 4 -iFILES sh -c 'read_distribution.py -i FILES -r /U5/Stefano/Genomes/UCSC/hg38_STAR_GencodeV24/hg38_UCSC_knownGene.bed > DISTRIBUTION/FILES.DistributionLog.txt;';
...
mkdir HTSEQ/
parallel -j 14 'samtools view {} | htseq-count -m intersection-strict -t exon -i gene_name -s reverse - /U5/Stefano/Genomes/UCSC/hg38_STAR_GencodeV24/genecode_hg38_protein.coding.gtf > {.}.txt' ::: *.UNIQUE.bam
...
rsem-calculate-expression --bam --estimate-rspd  --calc-ci --no-bam-output --seed 12345 -p 14 --ci-memory 30000 --forward-prob 0 $file /U5/Stefano/Genomes/UCSC/hg38_RSEM_GencodeV24/hg38 RSEM/"$newname" 
echo $file
echo $newname
done
